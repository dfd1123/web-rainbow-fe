...
import { SvgIcoTooltipArrow } from "@/components/icons";
...
const Tooltip = ({
  ...
  const checkValidPosition = useCallback(
    (sizeInfo: DomSizeInfoType) => {
      ...
      if (
        scrollY + targetTop - tooltipHeight - arrowHeight - space < scrollY &&
        place === "top"
      ) {
        return "bottom";
      }
      ...
      if (
        scrollY + targetTop + targetHeight + arrowHeight + space >
          scrollY + innerHeight &&
        place === "bottom"
      ) {
        return "top";
      ...
      if (
        innerWidth -
          (scrollX + targetLeft + targetWidth + arrowWidth + space + 10) <
          100 &&
        place === "right"
      ) {
        return "left";
  ...
  const getDomsSizeInfo: () => DomSizeInfoType = useCallback(() => {
    ...
    const { clientWidth: arrowWidth = 0, clientHeight: arrowHeight = 0 } =
      arrowRef.current ?? {};
    ...
    const bodyStyleTop =
      position === "fixed" ? Number(top.replace("px", "")) * -1 : 0;
    const bodyStyleLeft =
      position === "fixed" ? Number(left.replace("px", "")) * -1 : 0;
  ...
  const calculateTooltipStyle = useCallback(async () => {
    ...
    const { top: tooltipCustomTop = 0, left: tooltipCustomLeft = 0 } =
      tooltipPosition ?? {};
  ...
  return (
    <Portal id="tooltip">
      <div
        ...
        className={cx("tooltip", className, currentPlace, {
          open: tooltipOpen,
        })}
        style={
          {
