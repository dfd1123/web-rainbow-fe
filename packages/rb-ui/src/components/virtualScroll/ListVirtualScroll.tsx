...
const VirtualItem = memo(
  ({
    ...
    const setElementStyle = useCallback(() => {
      if (heightList[index]) {
        const top = heightList
          .slice(0, index)
          .reduce((arr, cur) => arr + cur, 0); // position top값을 계산
...
const ListVirtualScroll = <LIST extends UnknownArrayType>({
  ...
  const resizeObserver = useMemo(() => {
    return new ResizeObserver((entries) => {
      for (const entry of entries) {
        const { clientWidth, offsetHeight, dataset } =
  ...
  const maxHeight = useMemo(() => {
    ...
    return validHeightList.length
      ? Math.max(...heightList.filter((h) => !!h))
      : idealHeight;
  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: isKeepStatus && !list ? "100000px" : 0,
      }}
    >
