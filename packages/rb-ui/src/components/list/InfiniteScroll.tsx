...
type DefaultProps = {
  ...
  list: AllowAny[] | null;
  /** scroll을 하는 영역 요소 */
  scrollTarget?: HTMLElement | Document | RefObject<HTMLElement | Document>;
 ...
 * @param {AllowAny[]} [props.list=null] - 랜더링되고 있는 리스트 배열.
...
const InfiniteScroll = ({
  ...
  const totalCountRef = useRef(
    "totalCount" in props ? props.totalCount : undefined,
  );
  ...
  return (
    <div className={cx(className, "infinite-scroll")}>
      ...
      <div ref={target} className={cx("io-target")}>
        <div
          className={cx("infinite-scroll-loading", {
            active: loading && (list ?? []).length,
          })}
        >
