...
import { SvgIcoEllipsisChevron } from "@/components/icons";
...
const Ellipsis = ({
  ...
  const handleToggleShortEnd = useCallback(
  ...
  useLayoutEffect(() => {
    ...
    if (contentEl) {
      ...
      const isEnvChanged = observingEnvs.length
        ? observingEnvs.some(
            (env, index) => env !== prevObservingEnvs.current[index],
          )
        : false;
  ...
  return (
    ...
    >
      ...
      {triggerVisibile && (
        <button
          onClick={
            onShowMoreLessClick ? onShowMoreLessClick : handleToggleShortEnd
          }
