...
import {
  SvgIcoSortUpElement,
  SvgIcoSortDownElement,
  SvgIcoSortNormalElement,
...
const findTdColumns = (children: ReactNode): ReactNode[] => {
  ...
  Children.forEach(children, (child) => {
    if (isValidElement(child)) {
      const childElement = child as ReactElement<AllowAny>;

      if (childElement.type === Fragment) {
        result = [...result, ...findTdColumns(childElement.props.children)];
      } else if (childElement.type === TdColumn) {
        result.push(child);
      } else {
        result = [...result, ...findTdColumns(childElement.props.children)];
      }
...
const Table = <T extends TableData[]>({
  ...
  const tdColumns = useMemo(() => {
    ...
    return findTdColumns(children({ item: item as AllowAny, index: 0 }));
  }, [children, list]);
  ...
  const thRowData = useMemo(() => {
    const thRows = tdColumns?.map((column) => {
      ...
      return {
        ...
        enableSorting:
          typeof component.props?.sortValue !== "undefined" ||
          !!component.props?.handleClickSort,
        ...
        thRows
          ...
          .map((newTh) => {
            ...
            return {
              ...newTh,
              order:
                orderData?.fieldId === newTh.fieldId ? orderData?.order : "",
            };
          })
      );
    }
  ...
  const tableRows = useMemo(() => {
      ...
      Children.map(
        ...
        (child) => {
          if (isValidElement(child)) {
            ...
            if (childElement.type === TdColumn) {
              return child;
            } else {
              return Children.toArray(childElement.props.children);
            }
          }
          return [];
        },
    ...
    const sortValueList = componentList.reduce((acc, cur: AllowAny) => {
      const targetColumn = cur.find(
        (column: AllowAny) => column.props.fieldId === orderTargetFieldId,
      );
   ...
   */
  const handleThSorting = async (
    thData: TableThRowDataType & { index: number },
  ) => {
