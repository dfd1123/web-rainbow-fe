...
import { wait } from "@packages/rb-utils/etcUtils";
...
const TableRow = ({
    ...
    (newStatus?: boolean) => {
      if (expendTdEl.current) {
        ...
        if (expendDivEl) {
          ...
          expendDivEl.style.maxHeight = status
            ? `${expendDivEl.scrollHeight}px`
            : "0px";
        }
      }
    },
  ...
  const columnList = useMemo(() => {
     ...
     */
    const getChildren = (childArray: AllowAny[]): ReactNode[] => {
