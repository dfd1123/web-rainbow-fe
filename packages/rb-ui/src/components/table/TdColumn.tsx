...
const TdColumn = ({
  ...
  const [open, setOpen] = useState(expendOpen);

  const onExpendToggle = useCallback(
    async (e: MouseEvent<HTMLTableCellElement>) => {
      ...
      try {
        ...
        setTimeout(() => {
          handleExpendToggle?.(!open);
        }, 10);
      }
    },
    [expend, open, onClick, handleExpendToggle],
  );

  return (
    <td
      className={cx(className, fieldId, {
        "expend-controller": expend,
        "expend-open": expend && expendOpen,
      })}
      colSpan={colSpan}
      rowSpan={rowSpan}
      style={{ width: size }}
      onClick={onExpendToggle}
    >
