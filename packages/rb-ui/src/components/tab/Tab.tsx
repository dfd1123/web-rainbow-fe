...
interface PropsType {
   ...
   */
  tabNavScrollRef?: RefObject<HTMLElement>;
  /**
   ...
   */
  handleChange?: (key: AllowAny, name: string) => void;
  /**
   * tab 버튼을 클릭할때 호출되는 함수
   * @param key 선택된 tab의 key값
   * @param e  MouseEvent<HTMLButtonElement>
   */
  onClick?: (key: AllowAny, e: MouseEvent<HTMLButtonElement>) => void;
}
...
const Tab = ({
  ...
  const activeIndex = useMemo(() => {
    const index = filterTabItems.findIndex(
    ...
    if (swipeContent) {
      const handleChangeTabContent = () => {
        ...
        if (!disabled) {
          ...
          handleChange?.(swipeDirection === "next" ? nextKey : prevKey, name);
        }
      };
  ...
  const tabScrollSpy = useCallback(() => {
    ...
    // 선택된 tab button 요소
    const targetEl =
      tabBtnsRef.current.find(
        (btnEl) => btnEl && btnEl.classList.contains("active"),
    ...
    if (targetEl) {
      ...
      const scrollLeft =
        offsetLeft - tabNavEl.offsetWidth / 2 + clientWidth / 2;
