...
import { useCheckDevice } from "@packages/rb-hooks";
import { scrollFreeze, scrollRelease } from "@packages/rb-utils/browserUtils";
import { makeCxFunc } from "@packages/rb-utils/parserUtils";

import { SvgIcoClose, SvgIcoSearch } from "@/components/icons";
...
const SelectDropBox = forwardRef<HTMLDivElement, PropsType>((props, ref) => {
  ...
  const checkTopBottomSpace = useCallback(
    (top: number, clientHeight: number) => {
      ...
      if (direction === "top" && top - clientHeight + MARGIN < windowTop) {
        ...
        direction === "bottom" &&
        top + maxHeight + MARGIN > windowBottom
      ) {
        setBoxDirection("top");
      }
    },
  ...
  useEffect(() => {
    ...
    if (el && scrollEl) {
      ...
      if (typeof scrollEl.scrollTo === "function") {
        scrollEl.scrollTo(0, activeEl?.offsetTop ?? 0);
      }
  ...
  useEffect(() => {
    if (scrollAreaRef) {
      ...
      if (matchEl) {
        [...(el.children as AllowAny)].map((el) =>
      ...
      <div>
        ...
        {search && (
          <div className={cx("search-area")}>
            <SvgIcoSearch className={cx("ico-search")} />
            <input
              type="search"
            ...
            >
              <SvgIcoClose className={cx("ico-reset")} />
            </button>
          </div>
        )}
