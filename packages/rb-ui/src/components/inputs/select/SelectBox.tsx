...
import { useClickOutSide } from "@packages/rb-hooks";
import { makeCxFunc } from "@packages/rb-utils/parserUtils";

import { SvgIcoAccordion } from "@/components/icons";
 ...
 * @param {Function} [props.handleChange] - 값이 변경될 때 호출될 함수. `(value: AllowAny, name: AllowAny) => void`
 *
 * @example
 * <SelectBox
...
const SelectBox = forwardRef<SelectBoxRef, PropsType>((props, ref) => {
  const {
    ...
    noDataMsg = <NoData className={cx("no-data")} />,
    onClick,
    handleChange,
    ...inputProps
  } = props;
  ...
  const optionList = useMemo(() => {
    ...
    const onChangeForOption = ({ value: newValue }: SelectData) => {
      ...
      inputProps.onChange?.({ target: inpRef.current });
    ...
    const getChildren = (childArray: AllowAny[]): ReactElement[] => {
  ...
  const renderOptionList = useMemo(() => {
    return optionList.filter((option) => {
      return (option.props as { text: string }).text
        .toLowerCase()
        .includes(searchDirectText.toLowerCase());
    });
  ...
  const findOption = useMemo(() => {
    ...
    return optionList.find((option) => {
      const propsValue = (option.props as { value: string | number }).value;

      return propsValue === selectValue || propsValue === value;
    });
  ...
  const labelText = useMemo(() => {
    ...
    if (!findOption) {
      const props = optionList[0]?.props as {
        label?: string;
        children?: string;
      };

      return {
        text: placeholder ?? props.label ?? props.children ?? "",
        isPlaceholder: !!placeholder,
      };
    }

    const { label, children } = findOption.props as {
      label?: string;
      children?: string;
    };
  ...
  const handleEnter = useCallback(() => {
    ...
    const findOption = optionList.find((option) => {
      return (option.props as { text: string }).text.includes(
        searchText.toLowerCase(),
      );
    });

    if (findOption) {
      const findOptionProps = findOption.props as { value: string | number };

      setSelectValue(findOptionProps.value);
      handleChange?.(findOptionProps.value, name);

      inpRef.current.value = String(findOptionProps.value || "");
  ...
  const handleArrowKeyPress = useCallback(
    (type: "up" | "down") => {
      ...
      const findIndex = optionList.findIndex((option) => {
        ...
        return searchText
          ? optionProps.text === searchText
          : optionProps.value === selectValue;
      });

      if (type === "down") {
        // ⬇를 눌렀을 경우 findIndex에 1을 더해줘 option list에서 하나씩 아래로 내려가도록
        findOption =
          findIndex + 1 >= optionList.length
            ? optionList[0]
            : optionList[findIndex + 1];
      } else {
      ...
      const newText = (findOption.props as { text: string }).text;
  ...
  useClickOutSide(
    ...
    (e) => {
      ...
      if (!isRightBtn && !dropBoxRef.current?.contains(e.target as AllowAny))
        setOpen(false);
    },
    { offTouchEvent: true },
  );
  ...
  useEffect(() => {
    if (optionList.length) {
      if (!placeholder && !findOption) {
        ...
        const firstValue = (firstOption.props as { value: string | number })
          .value;
  ...
  return (
    <>
      ...
      >
        <label>
          <div
            className={cx("value", { placeholder: labelText.isPlaceholder })}
          >
