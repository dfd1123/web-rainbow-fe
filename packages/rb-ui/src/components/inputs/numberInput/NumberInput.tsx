...
import {
  ...
  makeCxFunc,
  makeParts,
} from "@packages/rb-utils/parserUtils";
import Decimal from "decimal.js";
import { last, replace } from "lodash";

import { SvgKeyboardArrowDown2, SvgKeyboardArrowUp2 } from "@/components/icons";
...
const inputModeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const zeroReplaceValue = replace(
    inputValue,
    /^0+[\d,]/gm,
    (match: string) => {
      return last(match);
...
const NumberInput = forwardRef<HTMLInputElement, Props>(
  (
    ...
    const stepIncreaseDecreaseRef = useRef<{
      increase: number;
      decrease: number;
    }>(null);
    const stepRef = useRef<number>(null);
      ...
      (number: string, step: number, type: "up" | "down") => {
        if (type === "up") {
          return new Decimal(number || 0)
            .add(
              new Decimal(
                stepIncreaseDecreaseRef.current?.increase ?? step ?? 1,
              ),
            )
          ...
          return new Decimal(number || 0)
            .sub(
              new Decimal(
                stepIncreaseDecreaseRef.current?.decrease ?? step ?? 1,
              ),
            )
            .toDP(digit, Decimal.ROUND_DOWN)
            .toString();
    ...
    const checkMinMaxValidate = useCallback(
      (value: string) => {
        ...
        if (
          ...
          new Decimal(Number(value)).lt(new Decimal(min))
        ) {
          return String(min);
        }
    ...
    const onChange = useCallback(
      ({
        ...
        const reg = !digit
          ? /^\d{0,18}$/
          : new RegExp(`^[0-9]*[.]{0,1}[0-9]{0,${digit}}$`, "gm");
        if (!putValue.match(reg)) return;

        if (
          typeof maxLength !== "undefined" &&
          (putValue || "").split(".")[0].replace(/[^0-9]/g, "").length >
            Number(maxLength)
        ) {
          return;
        }
        ...
        if (valueRef.current !== putValue) {
          handleChange?.(putValue, name, { type });
          if (e) {
    ...
    const handleClickCountBtn = useCallback(
      (type: "up" | "down") => {
        ...
        handleChange?.(valueRef.current, inputProps.name, { type });

        if (longPress.current) {
          window.clearTimeout(pressingTimeId.current);
          pressingTimeId.current = window.setTimeout(() => {
            handleClickCountBtn(type);
            if (delayTime.current > 100)
              delayTime.current -= delayTime.current / 5;
          }, delayTime.current);
        }
      },
      [
        ...
        <div className={cx("number-inp-cont")}>
          ...
          {!hideButton && (
            <div className={cx("number-btn-area")}>
              <Ripple
                {...rippleOption}
                disabled={checkDisabled || rippleOption?.disabled}
              >
                <button
                  data-type="up"
