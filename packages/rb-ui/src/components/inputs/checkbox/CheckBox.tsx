...
import { SvgIcoCheck, SvgIcoIndeterminate } from "@/components/icons";
  ...
  (props: Props, ref: ForwardedRef<HTMLInputElement>) => {
    ...
    const handleValueChange = useCallback(
      (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (Array.isArray(checkValue) && Array.isArray(value)) {
          handleChange?.(value.length > 0 ? [] : checkValue, name);
        ...
        if (checkBoxEls.length > 1 || Array.isArray(value)) {
          ...
          for (let i = 0; i < selectedEls.length; i += 1) {
            ...
            const indexOf = changeValue.findIndex(
              (val: unknown) => val === tempVal,
            );
          ...
          changeValue = currentData
            ? currentData.filter((d: string | number) =>
                changeValue.includes(d),
              )
            : uniq(changeValue);
          ...
          handleChange?.(checked ? checkValue : unCheckValue, name);
        }
    ...
    useEffect(() => {
      ...
      if (checkBoxEls.length > 1 || Array.isArray(value)) {
        ...
        if (Array.isArray(value)) {
          throw new Error(
            "단일의 name 값을 갖는 CheckBox는 배열 타입의 data를 가질 수 없습니다.",
          );
        }
      }
