 ...
 */
const MultiRangeInput = forwardRef<MultiRangeInputForwardRef, Props>(
  (props, ref) => {
    const {
      className = "",
      value,
    ...
    const onChangeSlide = useCallback(
      (
        ...
        if (
          showTextInput &&
          (!textInpLeftRef.current || !textInpRightRef.current)
        )
          return;
        ...
        if (direction === "left") {
          ...
          if (changeEmit) handleChange?.([putValue, range[1]], name);
          ...
          if (changeEmit) handleChange?.([range[0], putValue], name);
    ...
    const onMouseUp = useCallback(() => {
      if (
        range.some((number, index) => number !== preValueRef.current[index])
      ) {
