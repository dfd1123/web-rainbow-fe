...
import { SvgIcoInputReset, SvgIcoSearchDefault } from "@/components/icons";
...
interface PropsType extends InputHTMLAttributes<HTMLInputElement> {
  ...
  /** 엔터 키가 눌렸을 때 호출될 함수 */
  handleEnter?: (value: AllowAny, name?: string) => void;
  /** 리셋 버튼이 클릭됐을 때 호출될 함수 */
  handleReset?: (
    value: AllowAny,
    name?: string,
    clickResetBtn?: boolean,
  ) => void;
}
...
const TextInput = forwardRef<HTMLInputElement, PropsType>((props, ref) => {
  ...
  const { ...inputTagProps } = inputProps;
  ...
  const onFocus = useCallback(
    (e: FocusEvent<HTMLInputElement>, status: boolean) => {
      ...
      if (status) inputProps.onFocus?.(e);
      else inputProps.onBlur?.(e);
    },
    [inputProps],
  );
  ...
  return (
    ...
    >
      <div className={cx("input-area")}>
        {iconStatus.search && (
          <span className={cx("ico-prefix")}>{searchIcon}</span>
        )}
