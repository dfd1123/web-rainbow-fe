...
class SvgComponentGenerator {
  ...
  parseSvgListForType(list: string[]) {
    ...
    const particalSvgObj = fileList
      ...
      .reduce<Record<string, string>>((acc, cur) => {
        ...
        const directoryPascalName = _startCase(arr.join("-")).replace(
          / /gi,
          "",
        );
  ...
  async parseSvgListForFile(list: string[]) {
    ...
    const relativePath = path
      .relative(this.outputDir, this.svgFileDir)
      .replace(/\\/gi, "/");
  ...
  generate = _debounce(async () => {
    ...
    console.info("✨[Svg Component Generator] is Done!");
  }, 300);
}
